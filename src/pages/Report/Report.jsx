...
const Report = () => {
  const { auth } = useAuth();
  const { rows, columns, details, errors, onChange, generate_report } = useReport();
  ...
  return (
    <>
      <ViewTemplate
        ...
        topHeading={<TopHeading title="Reports" />}
      >
        <Grid container direction="column" mt={3} rowSpacing={3}>
          <Grid item>
            <FormControl>
              <FormLabel id="report_type">Report type</FormLabel>
              <RadioGroup row name="report_type" value={details?.report_type ?? ""} onChange={onChange}>
                <FormControlLabel value="Salaries report" control={<Radio />} label="Salaries report" />
                <FormControlLabel value="Daily Work report" control={<Radio />} label="Daily work report" />
                <FormControlLabel value="Attendance report" control={<Radio />} label="Attendance report" />
              </RadioGroup>
            </FormControl>
          </Grid>
          ...
          <Grid item>
            <Button value="Generate report" onClick={generate_report} />
          </Grid>
        </Grid>
      </ViewTemplate>
      <Snackbar />
